...
const EquipmentTab = () => {
    ...
    const renderGridView = () => (
        <div id="dashEquipList" className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {equipment.map(e => (
                <div key={e.id} className={`relative rounded-xl border p-4 bg-white ${selected.has(e.id) ? 'border-blue-500' : 'border-slate-200'}`}>
                    {user?.role === 'admin' && (
                        <>
                            <input 
                                type="checkbox"
                    ...
                    <div className="font-semibold pl-6 pr-16">{e.name}</div>
                    <div className="text-sm text-slate-500 pl-6">S/N: {e.serial}</div>
                    <div className="text-xs text-slate-400 mt-1 pl-6">{e.type}</div>
                ...
                <thead className="text-xs text-slate-700 uppercase bg-slate-50">
                    <tr>
                        <th scope="col" className="p-4">
                            {/* Master checkbox is now at the top */}
                        </th>
                ...
                <div className="xl:col-span-3 card rounded-2xl p-6 text-slate-900">
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            {user?.role === 'admin' && equipment.length > 0 && selected.size > 0 && (
                                <>
                            ...
                            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-3 py-2 rounded-lg border border-slate-200">
                                <option value="all">ว่าง & ถูกยืม</option>
                                <option value="available">ว่าง</option>
                                <option value="borrowed">ถูกยืม</option>
                                <option value="under_maintenance">ซ่อมบำรุง</option>
                                <option value="pending_repair_approval">รออนุมัติซ่อม</option>
